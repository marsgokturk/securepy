...
class UserDatabase:
    ...
    def delete_user(self, username):
        if username in self._users:
            ...
            logger.error(f"Attempted to delete non-existent user {username}")
...
def delete_account(user, username_to_delete, user_db):
    if not user.is_admin:
        logger.error("Unauthorized attempt to delete account.")
        raise PermissionError("Only admins can delete accounts!")

    user_db.delete_user(username_to_delete)
    send_notification(user, f"User {username_to_delete}'s account has been deleted.")
